...
function Note(props) {
    ...
    return (
        ...
        <div style={{display: (props.isCompleted && props.status===false) || (props.isActive && props.status===true) ? "none": "flex", backgroundColor: props.isDay? "hsl(0, 0%, 98%)": "hsl(235, 24%, 19%)", boxShadow: noteBoxShadow.boxShadow}} className="note_container">
          ...
          <div style={{background: props.status? "linear-gradient(hsl(192, 100%, 67%) , hsl(280, 87%, 65%))": "transparent"}} onClick={() => {props.handleChecks()}} className="circle">
          ...
          <p style={{textDecoration: props.status === false ? "none": "line-through", color: props.isDay ? dayFont.color: NightFont.color}}>{props.title}</p>
            ...
            <svg onClick={props.deleteNotes} className="cross" xmlns="http://www.w3.org/2000/svg" width="18" height="18"><path fill="#494C6B" fillRule="evenodd" d="M16.97 0l.708.707L9.546 8.84l8.132 8.132-.707.707-8.132-8.132-8.132 8.132L0 16.97l8.132-8.132L0 .707.707 0 8.84 8.132 16.971 0z"/></svg>
        </div>
